...
The official maintenance workspace keeps editor data in `StarMap/06_private/data/editor-state.local.json`; a standalone clone may set `STARMAP_PRIVATE_ROOT` or use its ignored `06_private/` fallback. This state records display order, hidden items, photo covers, and media order. Country creation uses one Chinese / English / ISO-code autocomplete field backed by the bundled country catalog, then derives the canonical names, code, flag, and map center from the selected result. City creation appears only after entering a country; the user enters a name and explicitly presses Search, then the editor uses that country's ISO code to constrain an online OpenStreetMap Nominatim lookup and derives the selected city's bilingual names and coordinates. Dates remain explicit user input. Hiding is non-destructive: source records and Inbox originals remain untouched.
